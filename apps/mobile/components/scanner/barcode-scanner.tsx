...
// Ekran boyutlarını al (kamera için explicit boyut gerekli)
const { width: SCREEN_WIDTH, height: SCREEN_HEIGHT } = Dimensions.get("window");

// Scanner config değerleri
...
export function BarcodeScanner({
  ...
  const handleBarcodeScanned = ({
    ...
    // Debounce: belirli süre içinde aynı barkod okunduysa sayıcıyı artır
    if (timeSinceLastScan < scanDebounceMs) {
      scanCountRef.current[data] = (scanCountRef.current[data] || 0) + 1;

      // Threshold'a ulaşıldı, onay ekranını göster
